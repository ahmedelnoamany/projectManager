...
export default class CalendarView extends Component {
  ...
  renderDays = () => {
    ...
    for (let i = 0; i < this.daysInMonth(); i++) {
      let today = i + 1 === parseInt(this.currentDay()) ? true : false;
  ...
  renderWeekView = () => {
    let daysInWeek = [];
    let activeDay = moment(this.year() + '-' + this.month() + '-' + this.currentDay()).day();
    let backCounter = activeDay;
    for (let i = 0; i < this.weekdays.length; i++) {
      let today = i === activeDay? true : false;
      let className = today? "calendar-view__week-day--today" : "calendar-view__week-day";
      //let labelClass = today? "calendar-view__month-day--date calendar-view__month-day--date-today" : "calendar-view__month-day--date calendar-view__month-day--date-current";
      daysInWeek.push(
        <div key={i} className={className}>
          <span 
          >{(this.currentDay() - backCounter) <= 31 ? (this.currentDay() - backCounter) : null}</span>
        </div>
      );
      backCounter--;
    }
    ...
    return (
      <div className="calendar-view">
        ...
        <div className="calendar-view__day-label-container">
          {(this.state.currentMode === 2 || this.state.currentMode === 1) && this.renderWeekdays()}
        </div>
        {
          (this.state.currentMode === 1) &&
            <div className="calendar-view__week-container">
              {this.renderWeekView()}
            </div>
        }
        {
